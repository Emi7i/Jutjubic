...
public class VideoEventPublisher {
    ...
    public void publishVideoUploadEvent(Videos video) {
        System.out.println("📤 Publishing video upload event for video ID: " + video.getId());
        ...
        System.out.println("✅ Event published to both queues");
        System.out.println("   JSON: " + jsonSize + " bytes, " + jsonSerializationTime + " μs");
        System.out.println("   Protobuf: " + protobufSize + " bytes, " + protobufSerializationTime + " μs");

        if (protobufSerializationTime < jsonSerializationTime) {
            double speedup = (double) jsonSerializationTime / protobufSerializationTime;
            System.out.println("   ⚡ Protobuf is " + String.format("%.2f", speedup) + "x faster");
        }

        double sizeReduction = ((double)(jsonSize - protobufSize) / jsonSize) * 100;
        System.out.println("   📦 Protobuf is " + String.format("%.2f", sizeReduction) + "% smaller");
    }
    ...
    private int publishAsJson(Videos video) {
        try {
            ...
            System.out.println("Error publishing JSON message: " + e.getMessage());
            return 0;
        }
    }
    ...
    private int publishAsProtobuf(Videos video) {
        try {
            ...
            System.out.println("Published Protobuf message: " + messageBytes.length + " bytes");
            return messageBytes.length;

        } catch (Exception e) {
            System.out.println("Error publishing Protobuf message: " + e.getMessage());
            return 0;
        }
    }
